---
title: "CUONG VO - Final Project - E Commerce"
output:
...
ggplot() +
  ...
  geom_point(data = state_list %>% arrange(Sales) %>% tail(10), aes(x=long, y=lat), size = 6, color = light_yellow) + 
...
ggplot() +
  ...
  geom_text_repel(data = state_list %>% arrange(Margin) %>% tail(10), 
                  aes(x=long, y=lat, label=toupper(State)), size = 4, color = dark_red) +
  geom_point(data = state_list %>% arrange(Margin) %>% tail(10), aes(x=long, y=lat), size = 6, color = light_yellow) + 
