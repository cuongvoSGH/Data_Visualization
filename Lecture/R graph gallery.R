...
df1 <- read.csv("https://raw.githubusercontent.com/holtzy/R-graph-gallery/master/DATA/dataConsumerConfidence.csv") %>% 
  mutate(date=lubridate::my(Time)) %>%  # Create the columns
  select(-Time) %>% # Drop Time columns
...
p1 <- df1 %>% ggplot()  # Create plot
p1 <- p1 + geom_hline(yintercept = 100,linetype="solid", size=.25) # Create constant hline = 100
p1 <- p1 + geom_point(data=df1 %>% 
               group_by(country) %>% 
               slice_max(date),
             aes(x=date, y=value, color=country),shape=16)  # Create end point for each line by using max function
p1 <- p1 + geom_line(aes(x=date, y=value, color=country)) # Create line chart
p1 <- p1 + gghighlight(use_direct_label = FALSE,
              unhighlighted_params = list(colour = alpha("grey85", 1))) 
p1 <- p1 + geom_text(data=df1 %>% 
              group_by(country) %>% 
              slice_max(date),
            aes(x=date, y=value, color=country, label = round(value)),
            hjust = -.5, vjust = .5, size=2.5, family=font, fontface="bold") 
p1 <- p1 + scale_color_met_d(name="Redon") 
p1 <- p1 + scale_x_date(date_labels = "%y")
p1 <- p1 + scale_y_continuous(breaks = c(90,95,100,105,110), labels = c("","","100","","")) 
p1 <- p1 + facet_wrap(~ country)
p1 <- p1 + facet_wrap(~  factor(country, levels=c('USA','China','Japan','Germany', 'UK','France', 'Italy', 'South Korea', 'Australia'))) 
